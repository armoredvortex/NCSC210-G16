...
// Wait for a specific child process pid to exit and return its pid.
// Return -1 if pid is not a child of this process or on error.
int
kwaitpid(int target_pid, uint64 addr)
{
  struct proc *pp;
  struct proc *p = myproc();
  ...
  for(;;){
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p && pp->pid == target_pid){
        has_target_child = 1;
        acquire(&pp->lock);
    ...
    // Requested pid is not our child.
    if(!has_target_child || killed(p)){
      release(&wait_lock);
      return -1;
    }

    // Wait until child state changes.
    sleep(p, &wait_lock);
  }
}
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE && p->frozen == 0) {
...
kkill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->pid == pid){
      p->killed = 1;
      // Ensure killed processes are schedulable so they can exit promptly.
      p->frozen = 0;
      if(p->state == SLEEPING){
